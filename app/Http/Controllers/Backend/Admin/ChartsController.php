...
class ChartsController extends Controller
{
    ...
    public function create()
    {
        ...
        else
        {
            ...
            return view('backend.charts.create')-> with('tests', $tests)->with('course_list',$course_list)
                ...
                ->with('textgroup_list',$textgroup_list)->with('question_row_list', $matrix_questions);
        }
    ...
    public function edit($id)
    {
        ...
        else
        {
            ...
            return view('backend.charts.edit')->with('chart',$chart)->with('current_tests',$current_tests)->with('tests', $tests)->with('course_list',$course_list)
                ...
                ->with('question_list',$question_list)->with('textgroup_list',$textgroup_list)->with('question_row_list', $matrix_questions);
        }
    ...
    function getMatrixInformation($content) {
        ...
        $rowcols = null;
        $index = -1;
        $is_checkbox = $this->isMatrixCheckbox($tempInputArray);
    ...
    function isMatrixCheckbox($tempInputArray) {
        $is_checkbox = false;
        foreach ($tempInputArray as $key => $value) {
            if ($key > 0) {
                $checkArray = explode('type="checkbox"', $value);
    ...
    public function get_info(Request $request)
    {
        ...
        if ($t_list !=null)
            ...
            {
                $tdata[$i]= DB::table('textgroups')->select("score","id","questiontype")->where('id','=',$t_list[$i])->get()->toArray();  
            }
        else
            $tdata="";
        $data= array(
